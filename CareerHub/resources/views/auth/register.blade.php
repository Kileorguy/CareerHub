...
<div class="w-screen min-h-screen flex justify-center py-12 bg-gradient-to-br from-background to-primary">
  <form action="/register" method="POST" class="w-96 h-fit bg-white rounded-lg p-10">
    @csrf
    <p class="font-bold text-2xl text-black mb-4">CareerHub</p>
    <select name="role" class="select select-bordered w-full text-md px-2 mb-4" id="role">
      <option value="Employee" {{ old('role')=='Employee' ? 'selected' : '' }}>Employee</option>
      <option value="Company" {{ old('role')=='Company' ? 'selected' : '' }}>Company</option>
    </select>
    <div id="employee-detail-form" class="hidden flex-col items-center justify-center gap-4 mb-4">
    ...
    <div id="company-detail-form" class="w-full hidden flex-col items-center gap-4 mb-4">
