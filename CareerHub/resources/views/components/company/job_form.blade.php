...
<dialog class="modal" data-job-form-id="{{ $type == 'Edit' ? 'edit-job-' . $job->id : 'add-job-form' }}">
  <div class="modal-box">
    ...
    <form class="flex flex-col justify-start items-start"
      ...
      <input class="input input-bordered w-full max-w-lg" id="job-description-{{ $type == 'Edit' ? $job->id : 'new' }}"
        name="job_description" type="text" placeholder="Input Job Description"
      ...
      <select class="select select-bordered w-full" id="job-level-{{ $type == 'Edit' ? $job->id : 'new' }}"
        name="job_level">
        <option value="Mid senior" {{ $type=='Edit' && $job->job_level == 'Mid senior' ? 'selected' : '' }}>Mid senior
        </option>
